...
public class ContrastComposite implements Composite, CompositeContext {
    public ContrastComposite(float brightness, int offset) {
    ...
    private int mixPixel(int x) {
        int xb = (x) & 0xFF;
        xb = (int) Math.min(255, Math.max(0, xb + offset) * brightness);
