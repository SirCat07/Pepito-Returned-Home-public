...
public class ReplaceComposite implements Composite, CompositeContext {
    ...
    public void compose(Raster src, Raster dstIn, WritableRaster dstOut) {
        ...
        for (y=0; y < height; y++) {
            ...
            for (x=0; x < width; x++) {
                int ya = (dstPixels[x] >> 24) & 0xFF;
                if(ya == 0)
                    continue;
                
                dstPixels[x] = mixPixel(srcPixels[x], ya);
            }
    ...
    private static int mixPixel(int x, int ya) {
        int xb = (x) & 0xFF;
        int xg = (x >> 8) & 0xFF;
        int xr = (x >> 16) & 0xFF;
        
        return (xb) | (xg << 8) | (xr << 16) | (ya << 24);
    }
